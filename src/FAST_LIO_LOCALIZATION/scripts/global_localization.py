...
def cb_save_cur_scan(pc_msg):
    ...
    pc_msg.header.frame_id = 'camera_init'   # odom
...
<<<<<<< HEAD
    ...
    while not rospy.is_shutdown() and initialized:
        try:
            # TODO 由于这里Fast lio发布的scan是已经转换到odom系下了 所以每次全局定位的初始解就是上一次的map2odom 不需要再拿odom了
            global_localization(T_map_to_odom)
            rate.sleep()
        except Exception as e:
            rospy.logerr("Localization thread error: %s", str(e))
            break

def initial_pose_callback(pose_msg):
    global initialized, initial_pose
    initial_pose = pose_to_mat(pose_msg)
    initialized = False  # 重置初始化状态[2,4](@ref)
    rospy.loginfo("Received new initial pose, triggering relocalization...")
    # 清除历史数据（可选）
    global cur_scan
    cur_scan = None  # 强制获取新扫描数据
    global cur_odom
    cur_odom = None
    ...
    while not rospy.is_shutdown():
        if not initialized and initial_pose is not None and cur_scan is not None:
            try:
                ...
                initialized = global_localization(initial_pose)
                if initialized:
                    rospy.loginfo('Localization succeeded!')
                    # 启动持续定位线程
                    _thread.start_new_thread(thread_localization, ())
                else:
                    rospy.logwarn('Localization failed, waiting new initial pose...')
                    initialized = False
                    initial_pose = None  # 重置等待新位姿
            except Exception as e:
                rospy.logerr("Localization error: %s", str(e))
                initialized = False
        rospy.sleep(0.1)
=======
    while not initialized:
